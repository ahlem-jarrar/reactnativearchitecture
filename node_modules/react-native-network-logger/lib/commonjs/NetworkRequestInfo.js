...
class NetworkRequestInfo {
  ...
  get curlRequest() {
    let headersPart = this.requestHeaders && Object.entries(this.requestHeaders).map(([key, value]) => "'".concat(key, ": ").concat(this.escapeQuotes(value), "'")).join(' -H ');
    headersPart = headersPart ? "-H ".concat(headersPart) : '';
    const body = this.dataSent && this.escapeQuotes(this.dataSent);
    const methodPart = this.method !== 'GET' ? "-X".concat(this.method.toUpperCase()) : '';
    const bodyPart = body ? "-d '".concat(body, "'") : '';
    const parts = ['curl', methodPart, headersPart, bodyPart, "'".concat(this.url, "'")];
  ...
  stringifyFormat(data) {
    try {
      ...
      if (data === null || data === void 0 ? void 0 : (_data$_parts = data._parts) === null || _data$_parts === void 0 ? void 0 : _data$_parts.length) {
